...
function getAppointmentsData(filterStatus) {
	...
	if (!id) {
		qryAppointments = query(collection(db, "appointments"), where("status", "==", filterStatus));
	}
	else if (id) {
		qryAppointments = query(collection(db, "appointments"), where("status", "==", filterStatus), where("userUid", "==", id));	
	}
