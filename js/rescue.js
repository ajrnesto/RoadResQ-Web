...
function getAppointmentsData(filterStatus) {
	...
	if (!id) {
		qryAppointments = query(collection(db, "rescue"), where("status", "==", filterStatus));
	}
	else if (id) {
		qryAppointments = query(collection(db, "rescue"), where("status", "==", filterStatus), where("userUid", "==", id));	
	}
